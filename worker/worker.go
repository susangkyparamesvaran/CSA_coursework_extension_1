...
func (e *GOLWorker) ProcessTurns(req WorkerRequest, res *WorkerResponse) error {
	...
	// start and end row for aws node
	startY := req.StartY
	endY := req.EndY
	...
	// split the sections node's rows into p.Threads
	workerSections := assignRows(rows, p.Threads)
	...
	for _, job := range workerSections {
		jobChan <- workerJob{
	...
	results := make([]workerResult, 0, len(workerSections))
	for i := 0; i < len(workerSections); i++ {
